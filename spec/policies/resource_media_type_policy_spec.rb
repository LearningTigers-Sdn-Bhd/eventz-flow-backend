# spec/policies/resource_media_type_policy_spec.rb
require 'rails_helper'

RSpec.describe ResourceMediaTypePolicy, type: :policy do
  ...
  permissions :index?, :show? do
    it "grants access to everyone" do
      expect(subject).to permit(org_owner, resource_media_type)
      expect(subject).to permit(writer, resource_media_type)
      expect(subject).to permit(regular_user, resource_media_type)
      expect(subject).to permit(visitor, resource_media_type)
    end
  end
