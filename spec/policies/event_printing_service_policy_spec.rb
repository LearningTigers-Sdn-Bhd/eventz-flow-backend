...
RSpec.describe EventPrintingServicePolicy, type: :policy do
  ...
  describe "scope" do
    ...
    context 'for event staff' do
      ...
      it 'returns event printing services for assigned events' do
        expect(Pundit.policy_scope(user, EventPrintingService).to_a).to match_array([staffed_event_printing_service])
      end
    end
    ...
    context 'for an exhibition contractor assigned to events' do
      ...
      it 'returns event printing services for assigned events' do
        expect(Pundit.policy_scope(user, EventPrintingService).to_a).to match_array([contractor_event_printing_service])
      end
    end

    context 'for other users' do
      it 'returns no event printing services' do
        expect(Pundit.policy_scope(create(:user), EventPrintingService).to_a).to be_empty
      end
    end
  end
end
