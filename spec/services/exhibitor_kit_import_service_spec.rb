...
RSpec.describe ExhibitorKitImportService do
  ...
  describe '.import' do
    ...
    def build_upload(rows, custom_headers: ['Custom: T Shirt Size'])
    ...
    it 'strips internal bookkeeping keys even if a stray Custom: column matches one' do
      file = build_upload(
        [[
        ...
        custom_headers: ['Custom: T Shirt Size', 'Custom: Booking Batch Id', 'Custom: Payment Option']
      )
    ...
    it 'dry_run: true validates without persisting' do
      file = build_upload([[
      ...
      expect {
        described_class.import(file.path, event: event, current_user: organizer, dry_run: true)
