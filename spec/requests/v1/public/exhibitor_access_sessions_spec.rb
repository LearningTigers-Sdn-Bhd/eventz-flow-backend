...
RSpec.describe 'Public exhibitor access', type: :request do
  ...
  describe 'POST /exhibitor_email_status' do
    it 'returns existing and sends access for a global user without exposing account details' do
      ...
      }.to change(PublicExhibitorAccessSession, :count).by(1)

      expect(response).to have_http_status(:accepted)
      expect(response.parsed_body).to eq('success' => true, 'data' => { 'existing' => true })
