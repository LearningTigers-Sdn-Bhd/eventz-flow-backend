class EventRentableItemPolicy < ApplicationPolicy
  def index?
    user.org_owner? || user.organizer? || (user.exhibition_contractor? && record&.event && user.exhibition_contractor_for?(record.event)) || (record&.event && user.is_event_staff?(record.event)) || user.exhibitor?
  end
  ...
  class Scope < Scope
    def resolve
      if user.org_owner? || user.organizer?
        ...
        scope.joins(:rentable_item) # Join rentable_items table
             .joins(event: :event_vendors) # Join through event to event_vendors table
             .where(rentable_items: { status: RentableItem.statuses[:active] })
             .where(event_vendors: { vendor_id: user.id, type: 'Exhibitor' }) # Filter EventVendors by user and type
