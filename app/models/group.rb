class Group < ApplicationRecord
  ...
  has_one :vendor, through: :group_affiliate
  ...
  # --- Scopes ---
  scope :with_vendor, -> { joins(:group_affiliate).distinct }
  scope :managed_by, ->(user) { joins(:group_members).where(group_members: { user_id: user.id, has_manager_access: true }) }
  scope :visible_to, ->(user) {
    if user&.org_owner?
      all
    elsif user&.vendor?
      joins(:group_affiliate).where(group_affiliates: { vendor_id: user.id }).distinct
    else
