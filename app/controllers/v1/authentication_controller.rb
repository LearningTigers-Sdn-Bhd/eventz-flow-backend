...
module V1
  class AuthenticationController < ApplicationController
    ...
    # DELETE /v1/logout
    def logout
      # 1. Revoke by refresh token if present (covers both authenticated and unauthenticated scenarios if cookie exists)
    ...
    def register_invited_vendor
      ...
      ActiveRecord::Base.transaction do
        ...
        if params[:vendor_profile].present? && @user.vendor_profile.present?
          ...
          # Handle image attachment separately
    ...
    def set_refresh_token_cookie(token, _expires_at_ignored)
      cookies.signed[:refresh_token] = {
        ...
        secure: Rails.env.production?,
        same_site: :lax,
        expires: JwtService::REFRESH_TOKEN_EXPIRATION.from_now
      }
    end
