  ...
  module Public
    class ExhibitorPaymentsController < ApplicationController
      ...
      def verify
        ...
        unless Payments::RazorpayGateway.valid_signature?(order_id: order_id, payment_id: payment_id, signature: signature)
          return render json: { success: false, message: "Invalid payment signature" }, status: :unprocessable_content
        end

        mark_exhibitor_kit_paid!(exhibitor_kit: exhibitor_kit, payment_id: payment_id, order_id: order_id, signature: signature)
      ...
      def callback
        ...
        if exhibitor_kit.paid?
          return redirect_to "#{frontend_url}/exhibitor-registration?step=success&kit=#{exhibitor_kit.id}", allow_other_host: true
        end
        ...
        unless Payments::RazorpayGateway.valid_signature?(order_id: order_id, payment_id: payment_id, signature: signature)
          return redirect_to "#{frontend_url}/exhibitor-registration?step=payment&error=invalid_signature&kit=#{exhibitor_kit.id}", allow_other_host: true
        end

        mark_exhibitor_kit_paid!(exhibitor_kit: exhibitor_kit, payment_id: payment_id, order_id: order_id, signature: signature)
        redirect_to "#{frontend_url}/exhibitor-registration?step=success&kit=#{exhibitor_kit.id}", allow_other_host: true
      rescue StandardError => e
        redirect_to "#{frontend_url}/exhibitor-registration?step=payment&error=#{CGI.escape(e.message)}", allow_other_host: true
      end
