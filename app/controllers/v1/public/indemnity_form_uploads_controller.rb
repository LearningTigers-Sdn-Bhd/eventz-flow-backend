...
module V1
  module Public
    class IndemnityFormUploadsController < ApplicationController
      ...
      def create
        ...
        if file.blank? || !file.respond_to?(:content_type)
          return render json: { success: false, message: 'Indemnity form is required' },
                        status: :unprocessable_content
        end
        unless ALLOWED_CONTENT_TYPES.include?(file.content_type)
          return render json: { success: false, message: 'Indemnity form must be a JPEG, PNG, WebP, or PDF' },
                        status: :unprocessable_content
        end
        if file.size.to_i > MAX_FILE_SIZE
          return render json: { success: false, message: 'Indemnity form is too large (max 20MB)' },
                        status: :unprocessable_content
        end
        ...
        blob = ActiveStorage::Blob.create_and_upload!(
          ...
          metadata: { document_key: 'indemnity_form', event_id: event.id, uploaded_at: Time.current.iso8601 }
        )
