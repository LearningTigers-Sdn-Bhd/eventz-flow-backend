module V1
  module BusinessMatching
    class BookingsController < ApplicationController
      ...
      def generate_report
        if params[:business_matching_event_id]
          service_result = BusinessMatchingService.new(current_user).fetch_bookings(
            params[:business_matching_event_id],
            params[:event_id],
            force_refresh: params[:force_refresh] == 'true'
          ...
          service_result = BusinessMatchingService.new(current_user).fetch_all_bookings(params[:event_id])
        end

        if service_result.success?
          bookings = service_result.data[:bookings]
          report_service = BusinessMatchingReportService.new(bookings, "Business Matching Report")
